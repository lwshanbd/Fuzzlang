"""DVCR: the method. Also serves as the template for DVCR−id / −structure / −loop."""
from __future__ import annotations

from collections.abc import Callable

from experiments.agent.policy_base import Policy, PolicyContext
from experiments.loop.search import RunResult, run_dvcr
from experiments.types import SIGNAL_FULL
from experiments.verifier.base import BaseVerifier


def make_dvcr_runner(
    ...
    *,
    ...
    The returned callable takes (source, compile_cmd) and runs one DVCR instance.
    ...
    def run(source: str, compile_cmd: list[str]) -> RunResult:
        return run_dvcr(source, compile_cmd, verifier, policy, ctx, T=T, K=K)

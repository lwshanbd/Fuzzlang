"""diagnostic repair causal ablations: −id, −structure, −loop.

All three share the diagnostic repair scaffold; they differ only in PolicyContext.signal_mode
and (for −loop) the T parameter. No new code paths in search.py.
"""
from __future__ import annotations

from collections.abc import Callable

from repair.agent.policy_base import Policy
from repair.loop.search import RunResult, run_repair_loop
from repair.methods.diag import make_diag_runner
from foundation.types import SIGNAL_NO_ID, SIGNAL_NO_STRUCT
from foundation.verifier.base import BaseVerifier


def make_no_id_runner(
    verifier: BaseVerifier, policy: Policy, **kw
) -> Callable[[str, list[str]], RunResult]:
    """Structure preserved, typed diag_id removed. Tests whether the integer ID
    itself carries causal weight beyond the structured {name, msg, span} bundle."""
    return make_diag_runner(verifier, policy, signal_mode=SIGNAL_NO_ID, **kw)


def make_no_structure_runner(
    verifier: BaseVerifier, policy: Policy, **kw
) -> Callable[[str, list[str]], RunResult]:
    """Raw stderr only (with DiagID lines stripped). Tests the structured
    interface as a whole. Equivalent to B1 in observation, but retains the
    diagnostic repair loop/search configuration so the only difference is the observation."""
    return make_diag_runner(verifier, policy, signal_mode=SIGNAL_NO_STRUCT, **kw)


def make_no_loop_runner(
    verifier: BaseVerifier, policy: Policy, *, K: int = 4, **kw
) -> Callable[[str, list[str]], RunResult]:
    """T=1 with K parallel proposals at first turn. Isolates the loop's
    contribution from the verifier-selection-at-first-turn contribution."""
    return make_diag_runner(verifier, policy, T=1, K=K, **kw)


def make_strict_zero_shot_runner(
    verifier: BaseVerifier, policy: Policy, **kw
) -> Callable[[str, list[str]], RunResult]:
    """T=1, K=1. The tightest single-sample with diagnostic repair's full observation,
    for direct head-to-head against B0 (which uses SIGNAL_NO_STRUCT)."""
    return make_diag_runner(verifier, policy, T=1, K=1, **kw)
